...
class CartPage {
  ...
  verifySelectedProductPrice(): this {
    cy.get<string>("@selectedProductPrice").then((price) => {
      cy.get(cartLocators.productItemPrice).should("contain.text", price);
    });
    return this;
  }
  ...
  verifySelectedProductDescription(): this {
    cy.get<string>("@selectedProductDescription").then((description) => {
      cy.get(cartLocators.productItemDescription).should(
        "contain.text",
        description
      );
    });
  ...
  verifySelectedProductNotInCart(): this {
    cy.get<string>("@selectedProduct").then((name) => {
      cy.contains(cartLocators.cartItemName, name).should("not.exist");
    });
